...
function ListActiveCharges($PermissionLevels, $LogInLevel, $DatabaseConnection)
{
    ...
    $query = 'SELECT `charges`.`ID`, `books`.`Title`, `users`.`Name`, `charges`.`BorrowDate`, `charges`.`ReturnDate` FROM `charges` LEFT JOIN `users` ON `charges`.`UserIdentifier` = `users`.`Identifier` LEFT JOIN `books` ON `charges`.`BookIdentifier` = `books`.`Identifier` WHERE `charges`.`Active` = true ORDER BY `charges`.`ID` LIMIT '.$Skip.', '.$Limit.';';
    $statement = $DatabaseConnection->prepare($query);
    if (!$statement)
        return array('response' => false, 'error' => 'Could not prepare statement in BookV2::'.__FUNCTION__, 'errorCode' => 3101);
    if (!$statement->execute())
